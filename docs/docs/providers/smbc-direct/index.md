...
`@repo/client-smbc-direct` is a provider client for SMBC Direct. After QR approval in the SMBC app, it can retrieve ordinary-deposit balances and transaction history.
...
```ts
import { loginWithPasskey } from '@repo/client-smbc-direct'
